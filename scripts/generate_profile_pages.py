...
<body>
  ...
  <main class="container">
    <div class="profile-page">
      <div class="profile-banner" style="background: linear-gradient(135deg, var(--accent) 0%, #6364ff 100%); position: relative; overflow: hidden;">
        <svg width="100%" height="100%" viewBox="0 0 1200 200" preserveAspectRatio="xMidYMid slice" style="position: absolute; top: 0; left: 0;">
          <defs>
          ...
          <g opacity="0.1">
            <circle cx="200" cy="50" r="80" fill="white"/>
            <circle cx="1000" cy="150" r="100" fill="white"/>
            <circle cx="600" cy="100" r="60" fill="white"/>
          </g>
          <text x="50%" y="50%" dominant-baseline="middle" text-anchor="middle" font-family="system-ui, -apple-system, sans-serif" font-size="48" font-weight="700" fill="white" opacity="0.9">Mastodon</text>
          <text x="50%" y="65%" dominant-baseline="middle" text-anchor="middle" font-family="system-ui, -apple-system, sans-serif" font-size="32" font-weight="600" fill="white" opacity="0.8">România</text>
        </svg>
      </div>
      ...
      <div style="margin: 40px 0; padding: 32px; background: var(--surface); border: 1px solid var(--border); border-radius: var(--radius-lg);">
        ...
        <div style="display: flex; flex-direction: column; gap: 16px;">
          <a href="{url}" target="_blank" rel="{rel_attr}" class="btn btn-primary" style="display: inline-flex; align-items: center; gap: 8px; color: #ffffff; font-weight: 700;">
      ...
      <div id="postsSection" style="margin: 40px 0; padding: 32px; background: var(--surface); border: 1px solid var(--border); border-radius: var(--radius-lg);">
        ...
        <button id="loadMoreBtn" style="display: none; margin-top: 24px; padding: 12px 24px; background: #4f50d4; color: #ffffff; border: none; border-radius: var(--radius); font-size: 16px; font-weight: 700; cursor: pointer; width: 100%; transition: all 0.2s ease;" onmouseover="this.style.background='#6364ff'; this.style.transform='translateY(-1px)'; this.style.boxShadow='0 4px 16px rgba(0, 0, 0, 0.4)';" onmouseout="this.style.background='#4f50d4'; this.style.transform=''; this.style.boxShadow='';" onclick="loadMorePosts()">
          Arată mai multe
        </button>
      </div>
  ...
  <script>
    ...
    function renderPost(post) {{
      ...
      content = content.replace(/(^|[^>\"'])(@([a-zA-Z0-9_]+)@([a-zA-Z0-9.-]+))/g, (match, before, fullMatch, username, instance) => {{
        ...
        // mstdn.ro: nofollow, social.5th.ro: dofollow (noopener only)
        const relAttr = instance === 'mstdn.ro' ? 'nofollow noopener' : 'noopener';
      ...
      content = content.replace(/<a\\s+([^>]*?)>/gi, (match, attrs) => {{
        ...
        // Skip if it's already an internal profile link we just created (they already have correct rel)
        if (href && href.startsWith('/profiles/')) {{
          return match; // Keep as is, already processed
        }}
...
def main():
    ...
    for profile in profiles:
        ...
        if avatar:
            avatar_html = f'<img src="{html.escape(avatar)}" alt="{display_name}" width="120" height="120" loading="eager" decoding="async" fetchpriority="high" style="width:100%;height:100%;object-fit:cover;border-radius:var(--radius-lg);" onerror="this.style.display=\'none\'; this.nextElementSibling.style.display=\'flex\';">\n          <span style="display:none;">{display_name[0].upper() if display_name else "?"}</span>'
        else:
