...
<body>
  ...
  <script>
    ...
    async function loadPosts(offset = 0, limit = 10) {{
      try {{
        const apiBase = `https://${{profileData.instance}}/api/v1`;
        const lookupUrl = `${{apiBase}}/accounts/lookup`;
        const lookupResponse = await fetch(`${{lookupUrl}}?acct=${{encodeURIComponent(profileData.username)}}`);
        ...
        const statusesUrl = `${{apiBase}}/accounts/${{accountId}}/statuses`;
        const statusesResponse = await fetch(`${{statusesUrl}}?limit=${{limit}}&offset=${{offset}}&exclude_replies=true&exclude_reblogs=true`);
        
        if (!statusesResponse.ok) {{
          throw new Error('Failed to load statuses');
        }}
        
        return await statusesResponse.json();
      }} catch (error) {{
    ...
    function addNofollowToLinks(html) {{
      return html.replace(/<a\\s+([^>]*?)>/gi, (match, attrs) => {{
        if (!attrs.includes('rel=')) {{
          ...
          return `<a ${{attrs.replace(/rel="([^"]*)"/, 'rel="$1 nofollow noopener")}}>`;
        }}
        return match;
      }});
    }}
    ...
    function renderPost(post) {{
      const content = addNofollowToLinks(post.content || '');
      ...
      return `
        <div class="post-item" style="padding: 20px; background: var(--bg); border: 1px solid var(--border); border-radius: var(--radius-lg);">
          <div style="display: flex; gap: 12px; margin-bottom: 12px;">
            <div style="flex: 1;">
              <div style="font-size: 14px; color: var(--text-muted); margin-bottom: 8px;">
                <a href="${{postUrl}}" target="_blank" rel="nofollow noopener" style="color: var(--text-muted); text-decoration: none;">
                  ${{createdAt}}
                </a>
              </div>
          ...
          <div style="display: flex; gap: 16px; margin-top: 12px; padding-top: 12px; border-top: 1px solid var(--border);">
            <a href="${{postUrl}}" target="_blank" rel="nofollow noopener" style="display: inline-flex; align-items: center; gap: 6px; color: var(--text-muted); text-decoration: none; font-size: 14px;">
